...
# EventHallusion eval on N local vLLM replicas (one per GPU).
# Mirrors scripts/run_ovo_8gpu_vllm.sh / dataset3/scripts/run_videohallucer_8gpu.sh.
#
# Usage:
#   ./dataset3/scripts/run_eventhallusion_8gpu.sh <model_path> <policy>
#     policy: anchor_delta | delta_only
#
# Examples:
#   ./dataset3/scripts/run_eventhallusion_8gpu.sh /mmu_mllm_hdd/Models/Qwen3-VL-8B-Instruct anchor_delta
#
# Env overrides (same as VideoHallucer script):
#   GPUS / PORT_BASE / WORKERS / RESUME / ALLOW_EXISTING_SERVERS / LIMIT / TASK / RUN_NAME / OUTPUT_DIR / PYTHON / VLLM
...
BASE_CONFIG="dataset3/configs/eval_eventhallusion_${POLICY}.yaml"
...
RUN_NAME="${RUN_NAME:-eh_${POLICY}_${model_basename}_${date_tag}}"
