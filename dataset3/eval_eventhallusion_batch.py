#!/usr/bin/env python3
"""Multi-process batch evaluation for EventHallusion.

Mirrors dataset3/eval_videohallucer_batch.py: worker_id % len(endpoints) routes
each worker to one endpoint; RolloutRunner.run_multi_qa_sample shares one prefix
rollout across multiple last-step questions; --resume skips entries whose qa_ids
are all already in shard files.
...
def _split_endpoint_arg(values: Any) -> list[str]:
    """Accept --endpoints as space-separated args OR comma-separated string."""
    out: list[str] = []
    if not values:
        return out
...
def _load_completed(shard_dir: Path) -> dict[str, dict[str, dict[str, Any]]]:
    """Return {source_id: {qa_id: row}} for resume."""
        ...
        with part_path.open(encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                completed.setdefault(row.get("source_id", ""), {})[row.get("qa_id", "")] = row
    return completed
...
def _entry_completed(entry_qa_list: list[dict[str, Any]], completed: dict[str, dict[str, dict[str, Any]]]) -> bool:
    if not entry_qa_list:
        return True
    source_id = entry_qa_list[0]["source_id"]
    by_qa = completed.get(source_id, {})
    return all(qa["qa_id"] in by_qa for qa in entry_qa_list)
        ...
        with part_path.open(encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                key = (row.get("source_id", ""), row.get("qa_id", ""))
                if key in seen:
                    continue
                seen.add(key)
    ...
    with open(part_path, part_mode, encoding="utf-8") as f:
        while True:
            ...
            try:
                ...
                traceback.print_exc()
                for qa in qa_list:
                    f.write(json.dumps(_error_row(sample, qa, err), ensure_ascii=False) + "\n")
                f.flush()
            ...
            if multi.task_failed:
                for qa in qa_list:
                    f.write(json.dumps(_error_row(sample, qa, multi.failure_reason or "task_failed"), ensure_ascii=False) + "\n")
                f.flush()
                _PROGRESS_QUEUE.put({"type": "sample_done", "worker_id": worker_id, "sample_id": sample.sample_id, "error": True})
                continue
