...
RUN_NAME="${RUN_NAME:-exp3_grppo_smoke_debug}"
...
TRAIN_FILE="${TRAIN_FILE:-${DATASET_ROOT}/rl_exp3.jsonl}"
VAL_FILE="${VAL_FILE:-${DATASET_ROOT}/rl_exp3.jsonl}"

SOURCE_MODEL_PATH="${SOURCE_MODEL_PATH:-${V5_DIR}/models/qwen3vl8b_streamweave_sft_answered_full_anchor_delta_init_anchor_step200_vllm}"
...
GRPPO_ANSWER_DECAY="${GRPPO_ANSWER_DECAY:-0.4}"
GRPPO_PROCESS_WEIGHT="${GRPPO_PROCESS_WEIGHT:-1.0}"
GRPPO_FORMAT_WEIGHT="${GRPPO_FORMAT_WEIGHT:-0.1}"
GRPPO_STEP_WEIGHT="${GRPPO_STEP_WEIGHT:-1.0}"
GRPPO_ANSWER_WEIGHT="${GRPPO_ANSWER_WEIGHT:-0.3}"
GRPPO_NORM_BY_STD="${GRPPO_NORM_BY_STD:-true}"
GRPPO_MIN_STD="${GRPPO_MIN_STD:-0.07}"
GRPPO_FILTER_GROUPS_ENABLE="${GRPPO_FILTER_GROUPS_ENABLE:-true}"
GRPPO_FILTER_MIN_STD="${GRPPO_FILTER_MIN_STD:-0.07}"

TRACE_SAMPLE_EVERY="${TRACE_SAMPLE_EVERY:-1}"
TRACE_TRAJ_INDEX="${TRACE_TRAJ_INDEX:-0}"
TRACE_MAX_CHARS="${TRACE_MAX_CHARS:-6000}"
DEBUG_GROUPS="${DEBUG_GROUPS:-2}"
DEBUG_TRAJS="${DEBUG_TRAJS:-2}"
...
if [[ "${STREAMWEAVE_ALLOW_EXISTING_RL:-0}" != "1" ]] && pgrep -f "verl.trainer.main_ppo" >/dev/null 2>&1; then
    ...
    echo "Refusing to start smoke because this script stops Ray before launch." >&2
...
echo "StreamWeave exp3 smoke groups=${TRAIN_BATCH_SIZE} traj_per_group=${ROLLOUT_N} total_training_steps=${TOTAL_TRAINING_STEPS}"
echo "StreamWeave exp3 grppo reward process_weight=${GRPPO_PROCESS_WEIGHT} format_weight=${GRPPO_FORMAT_WEIGHT}"
echo "StreamWeave exp3 grppo answer_decay=${GRPPO_ANSWER_DECAY} step_weight=${GRPPO_STEP_WEIGHT} answer_weight=${GRPPO_ANSWER_WEIGHT}"
echo "StreamWeave exp3 grppo step_filter enable=${GRPPO_FILTER_GROUPS_ENABLE} min_std=${GRPPO_FILTER_MIN_STD}"
echo "StreamWeave trace sample_every=${TRACE_SAMPLE_EVERY} traj_index=${TRACE_TRAJ_INDEX}"
