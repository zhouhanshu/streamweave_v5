...
for arg in "$@"; do
    case "${arg}" in
        ...
        data.streamweave.reward.judge.backend=*)
            JUDGE_BACKEND="${arg#*=}"
            ;;
        algorithm.rlmlr_alpha=*)
            RLMLR_ALPHA="${arg#*=}"
            ;;
        algorithm.rlmlr_norm_by_std=*)
            RLMLR_NORM_BY_STD="${arg#*=}"
            ;;
        algorithm.rlmlr_outcome_key=*)
...
ALGO_ARGS=(
    ...
    # RLMLR-specific fields are not part of stepwise.yaml's algorithm schema,
    # so they need the `+` prefix to add them to the resolved config.
    ...
    +algorithm.rlmlr_state_components='[step_score,format_score]'
    +algorithm.rlmlr_state_weights='[1.0,1.0]'
    algorithm.use_kl_in_reward=false
    # DAPO group filter OFF by default for RLMLR: it keeps groups by
    # trajectory_score variance, which would discard exactly the groups where
    # RLMLR can still produce per-step state signal despite same final outcome.
    # Set this to true on the CLI only if you understand the trade-off.
