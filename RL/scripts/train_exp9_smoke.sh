...
TOTAL_TRAINING_STEPS="${TOTAL_TRAINING_STEPS:-18}"
TOTAL_EPOCHS="${TOTAL_EPOCHS:-1}"

GRPPO_ANSWER_DECAY="${GRPPO_ANSWER_DECAY:-0.3}"
GRPPO_PROCESS_WEIGHT="${GRPPO_PROCESS_WEIGHT:-0.7}"
GRPPO_FORMAT_WEIGHT="${GRPPO_FORMAT_WEIGHT:-0.15}"
GRPPO_NOTE_FREQUENCY_WEIGHT="${GRPPO_NOTE_FREQUENCY_WEIGHT:-0.15}"
GRPPO_STEP_WEIGHT="${GRPPO_STEP_WEIGHT:-1.0}"
GRPPO_ANSWER_WEIGHT="${GRPPO_ANSWER_WEIGHT:-0.6}"
GRPPO_NORM_BY_STD="${GRPPO_NORM_BY_STD:-true}"
GRPPO_MIN_STD="${GRPPO_MIN_STD:-0.01}"
GRPPO_FILTER_GROUPS_ENABLE="${GRPPO_FILTER_GROUPS_ENABLE:-true}"
GRPPO_FILTER_MIN_STD="${GRPPO_FILTER_MIN_STD:-0.01}"
GRPPO_ANSWER_EVENT_MODE="${GRPPO_ANSWER_EVENT_MODE:-timeline}"
GRPPO_SILENCE_REWARD="${GRPPO_SILENCE_REWARD:-true}"
GRPPO_SILENCE_REWARD_VALUE="${GRPPO_SILENCE_REWARD_VALUE:-0.05}"
...
TRACE_ALL_TRAJS="${TRACE_ALL_TRAJS:-1}"
TRACE_MAX_CHARS="${TRACE_MAX_CHARS:-20000}"
TRACE_JUDGE_MAX_CHARS="${TRACE_JUDGE_MAX_CHARS:-8000}"
...
RUNTIME_ENV=(
    ...
    STREAMWEAVE_TRACE_ALL_TRAJS="${TRACE_ALL_TRAJS}"
    STREAMWEAVE_TRACE_MAX_CHARS="${TRACE_MAX_CHARS}"
    STREAMWEAVE_TRACE_JUDGE_MAX_CHARS="${TRACE_JUDGE_MAX_CHARS}"
...
echo "StreamWeave ${RUN_NAME} actor lr=${ACTOR_LR}"
...
echo "StreamWeave trace sample_every=${TRACE_SAMPLE_EVERY} all_trajs=${TRACE_ALL_TRAJS} traj_index=${TRACE_TRAJ_INDEX} trace_max_chars=${TRACE_MAX_CHARS} judge_max_chars=${TRACE_JUDGE_MAX_CHARS} debug_groups=${DEBUG_GROUPS} debug_trajs=${DEBUG_TRAJS}"
