...
if [[ -n "${EXP9_24_RAY_ROLE:-}" && -z "${EXP9_RAY_ROLE:-}" ]]; then
    EXP9_RAY_ROLE="${EXP9_24_RAY_ROLE}"
else
...
start_ray_head() {
    if [[ -z "${RAY_HEAD_IP}" ]]; then
        echo "EXP9_24_RAY_ROLE=head requires RAY_HEAD_IP=<head-node-ip>." >&2
        exit 2
    fi
...
start_ray_worker() {
    if [[ -z "${RAY_HEAD_IP}" ]]; then
        echo "EXP9_24_RAY_ROLE=worker requires RAY_HEAD_IP=<head-node-ip>." >&2
        exit 2
    fi
    local node_ip_args=()
...
case "${EXP9_RAY_ROLE}" in
    head)
        echo "Starting exp9_24 Ray head at ${RAY_HEAD_IP}:${RAY_PORT}"
        start_ray_head
        exit 0
        ;;
    worker)
        echo "Starting exp9_24 Ray worker, connecting to ${RAY_HEAD_IP}:${RAY_PORT}"
        start_ray_worker
        exit 0
        ;;
    driver)
        ;;
    *)
        echo "Unknown EXP9_24_RAY_ROLE/EXP9_RAY_ROLE=${EXP9_RAY_ROLE}; expected head, worker, or driver." >&2
        exit 2
        ;;
esac
...
if [[ ! -f "${TRAIN_FILE}" || ! -f "${VAL_FILE}" ]]; then
    echo "Missing exp9_24 data file:" >&2
...
echo "StreamWeave exp9_24 adv_estimator=${ADV_ESTIMATOR}"
echo "StreamWeave exp9_24 judge prompt_version=${JUDGE_PROMPT_VERSION}"
echo "StreamWeave exp9_24 grppo reward process_weight=${GRPPO_PROCESS_WEIGHT} format_weight=${GRPPO_FORMAT_WEIGHT} note_frequency_weight=${GRPPO_NOTE_FREQUENCY_WEIGHT}"
echo "StreamWeave exp9_24 grppo answer_decay=${GRPPO_ANSWER_DECAY} step_weight=${GRPPO_STEP_WEIGHT} answer_weight=${GRPPO_ANSWER_WEIGHT} norm_by_std=${GRPPO_NORM_BY_STD}"
echo "StreamWeave exp9_24 grppo answer_event_mode=${GRPPO_ANSWER_EVENT_MODE} silence_reward=${GRPPO_SILENCE_REWARD} silence_reward_value=${GRPPO_SILENCE_REWARD_VALUE} forced_postprocess=${GRPPO_FORCED_ANSWER_POSTPROCESS_ENABLE}"
echo "StreamWeave exp9_24 grppo target_reward answer_weight=${GRPPO_TARGET_ANSWER_WEIGHT} format_weight=${GRPPO_TARGET_FORMAT_WEIGHT} validation_score_key=${STEPWISE_VALIDATION_SCORE_KEY}"
