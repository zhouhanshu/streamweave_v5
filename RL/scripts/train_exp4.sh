...
echo "StreamWeave exp4 adv_estimator=${ADV_ESTIMATOR}"
echo "StreamWeave exp4 judge prompt_version=${JUDGE_PROMPT_VERSION}"
echo "StreamWeave exp4 grppo reward process_weight=${GRPPO_PROCESS_WEIGHT} format_weight=${GRPPO_FORMAT_WEIGHT}"
echo "StreamWeave exp4 grppo answer_decay=${GRPPO_ANSWER_DECAY} step_weight=${GRPPO_STEP_WEIGHT} answer_weight=${GRPPO_ANSWER_WEIGHT} norm_by_std=${GRPPO_NORM_BY_STD}"
echo "StreamWeave exp4 grppo step_filter enable=${GRPPO_FILTER_GROUPS_ENABLE} min_std=${GRPPO_FILTER_MIN_STD}"
echo "StreamWeave exp4 scale train_batch=${TRAIN_BATCH_SIZE} gen_batch=${GEN_BATCH_SIZE} rollout.n=${ROLLOUT_N} max_steps=${MAX_STEPS}"
