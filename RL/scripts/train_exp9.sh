...
TRAIN_FILE="${TRAIN_FILE:-${DATASET_ROOT}/rl_0516_train.jsonl}"
VAL_FILE="${VAL_FILE:-${DATASET_ROOT}/rl_0515_val.jsonl}"

SOURCE_MODEL_PATH="${SOURCE_MODEL_PATH:-${V5_DIR}/models/qwen3_rl_exp8_step100}"
...
start_ray_head() {
    if [[ -z "${RAY_HEAD_IP}" ]]; then
        echo "EXP9_RAY_ROLE=head requires RAY_HEAD_IP=<head-node-ip>." >&2
        exit 2
    fi
...
start_ray_worker() {
    if [[ -z "${RAY_HEAD_IP}" ]]; then
        echo "EXP9_RAY_ROLE=worker requires RAY_HEAD_IP=<head-node-ip>." >&2
        exit 2
    fi
    local node_ip_args=()
...
case "${EXP9_RAY_ROLE}" in
    head)
        echo "Starting exp9 Ray head at ${RAY_HEAD_IP}:${RAY_PORT}"
        start_ray_head
        exit 0
        ;;
    worker)
        echo "Starting exp9 Ray worker, connecting to ${RAY_HEAD_IP}:${RAY_PORT}"
        start_ray_worker
        exit 0
        ;;
    driver)
        ;;
    *)
        echo "Unknown EXP9_RAY_ROLE=${EXP9_RAY_ROLE}; expected head, worker, or driver." >&2
        exit 2
        ;;
esac
...
echo "StreamWeave exp9 adv_estimator=${ADV_ESTIMATOR}"
echo "StreamWeave exp9 judge prompt_version=${JUDGE_PROMPT_VERSION}"
echo "StreamWeave exp9 grppo reward process_weight=${GRPPO_PROCESS_WEIGHT} format_weight=${GRPPO_FORMAT_WEIGHT} note_frequency_weight=${GRPPO_NOTE_FREQUENCY_WEIGHT}"
echo "StreamWeave exp9 grppo answer_decay=${GRPPO_ANSWER_DECAY} step_weight=${GRPPO_STEP_WEIGHT} answer_weight=${GRPPO_ANSWER_WEIGHT} norm_by_std=${GRPPO_NORM_BY_STD}"
echo "StreamWeave exp9 grppo answer_event_mode=${GRPPO_ANSWER_EVENT_MODE} silence_reward=${GRPPO_SILENCE_REWARD} silence_reward_value=${GRPPO_SILENCE_REWARD_VALUE} forced_postprocess=${GRPPO_FORCED_ANSWER_POSTPROCESS_ENABLE}"
echo "StreamWeave exp9 grppo target_reward answer_weight=${GRPPO_TARGET_ANSWER_WEIGHT} format_weight=${GRPPO_TARGET_FORMAT_WEIGHT} validation_score_key=${STEPWISE_VALIDATION_SCORE_KEY}"
echo "StreamWeave exp9 actor_kl use=${ACTOR_USE_KL_LOSS} coef=${ACTOR_KL_LOSS_COEF} type=${ACTOR_KL_LOSS_TYPE}"
echo "StreamWeave exp9 grppo step_filter enable=${GRPPO_FILTER_GROUPS_ENABLE} min_std=${GRPPO_FILTER_MIN_STD}"
echo "StreamWeave exp9 scale train_batch=${TRAIN_BATCH_SIZE} gen_batch=${GEN_BATCH_SIZE} val_batch=${VAL_BATCH_SIZE} val_max_samples=${VAL_MAX_SAMPLES} rollout.n=${ROLLOUT_N} max_steps=${MAX_STEPS}"
echo "StreamWeave exp9 actor lr=${ACTOR_LR} resume_mode=${RESUME_MODE} save_freq=${SAVE_FREQ} test_freq=${TEST_FREQ}"
echo "StreamWeave exp9 ray role=${EXP9_RAY_ROLE} address=${DRIVER_RAY_ADDRESS} nnodes=${NNODES} n_gpus_per_node=${N_GPUS_PER_NODE} agent_workers=${AGENT_NUM_WORKERS} object_store_memory=${RAY_OBJECT_STORE_MEMORY}"
echo "StreamWeave exp9 hardware gpu_ids=${GPU_IDS}"
echo "StreamWeave exp9 micro ppo=${PPO_MICRO_BATCH_SIZE_PER_GPU} logprob=${LOGPROB_MICRO_BATCH_SIZE_PER_GPU} rollout_max_num_seqs=${ROLLOUT_MAX_NUM_SEQS}"
