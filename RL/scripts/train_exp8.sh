...
RAY_TMPDIR="/tmp/swray_exp8_$$"

DATASET_ROOT="${V5_DIR}/dataset2"
DATASET_NAME="mixed_rl_exp3"
TRAIN_FILE="${DATASET_ROOT}/rl_0515_train.jsonl"
VAL_FILE="${DATASET_ROOT}/rl_0515_val.jsonl"

SOURCE_MODEL_PATH="${V5_DIR}/models/qwen_sft_0513"

GPU_IDS="${CUDA_VISIBLE_DEVICES:-0,1,2,3,4,5,6,7}"
EXP8_RAY_ROLE="${EXP8_RAY_ROLE:-driver}"
NNODES="${NNODES:-2}"
...
RAY_OBJECT_STORE_MEMORY="${RAY_OBJECT_STORE_MEMORY:-28000000000}"
...
case "${JUDGE_ENABLE,,}" in
    true|1|yes|on)
        JUDGE_ENABLED_NORMALIZED=true
        ;;
    *)
        JUDGE_ENABLED_NORMALIZED=false
        ;;
esac

if [[ "${JUDGE_ENABLED_NORMALIZED}" == "true" && "${JUDGE_BACKEND}" == "gemini" ]]; then
...
if [[ "${EXP8_RAY_ROLE}" == "driver" && "${STREAMWEAVE_ALLOW_EXISTING_RL:-0}" != "1" ]] && pgrep -f "verl.trainer.main_ppo" >/dev/null 2>&1; then
...
start_ray_head() {
    if [[ -z "${RAY_HEAD_IP}" ]]; then
        echo "EXP8_RAY_ROLE=head requires RAY_HEAD_IP=<head-node-ip>." >&2
        exit 2
    fi
...
start_ray_worker() {
    if [[ -z "${RAY_HEAD_IP}" ]]; then
        echo "EXP8_RAY_ROLE=worker requires RAY_HEAD_IP=<head-node-ip>." >&2
        exit 2
    fi
    env "${RUNTIME_ENV[@]}" "${RAY_BIN}" stop --force >/dev/null 2>&1 || true
    env "${RUNTIME_ENV[@]}" "${RAY_BIN}" start \
...
case "${EXP8_RAY_ROLE}" in
    head)
        echo "Starting exp8 Ray head at ${RAY_HEAD_IP}:${RAY_PORT}"
        start_ray_head
        exit 0
        ;;
    worker)
        echo "Starting exp8 Ray worker, connecting to ${RAY_HEAD_IP}:${RAY_PORT}"
        start_ray_worker
        exit 0
        ;;
    driver)
        ;;
    *)
        echo "Unknown EXP8_RAY_ROLE=${EXP8_RAY_ROLE}; expected head, worker, or driver." >&2
        exit 2
        ;;
esac
...
if [[ "${JUDGE_ENABLED_NORMALIZED}" == "true" && "${JUDGE_BACKEND}" == "gemini" && ! -f "${GEMINI_CREDENTIALS}" ]]; then
...
echo "StreamWeave exp8 ray role=${EXP8_RAY_ROLE} address=${DRIVER_RAY_ADDRESS} nnodes=${NNODES} n_gpus_per_node=${N_GPUS_PER_NODE} agent_workers=${AGENT_NUM_WORKERS}"
echo "StreamWeave exp8 adv_estimator=${ADV_ESTIMATOR}"
echo "StreamWeave exp8 judge prompt_version=${JUDGE_PROMPT_VERSION}"
echo "StreamWeave exp8 grppo reward process_weight=${GRPPO_PROCESS_WEIGHT} format_weight=${GRPPO_FORMAT_WEIGHT} note_frequency_weight=${GRPPO_NOTE_FREQUENCY_WEIGHT}"
echo "StreamWeave exp8 grppo answer_decay=${GRPPO_ANSWER_DECAY} step_weight=${GRPPO_STEP_WEIGHT} answer_weight=${GRPPO_ANSWER_WEIGHT} norm_by_std=${GRPPO_NORM_BY_STD}"
echo "StreamWeave exp8 grppo answer_event_mode=${GRPPO_ANSWER_EVENT_MODE} silence_reward=${GRPPO_SILENCE_REWARD} silence_reward_value=${GRPPO_SILENCE_REWARD_VALUE} forced_postprocess=${GRPPO_FORCED_ANSWER_POSTPROCESS_ENABLE}"
echo "StreamWeave exp8 grppo target_reward answer_weight=${GRPPO_TARGET_ANSWER_WEIGHT} format_weight=${GRPPO_TARGET_FORMAT_WEIGHT} validation_score_key=${STEPWISE_VALIDATION_SCORE_KEY}"
echo "StreamWeave exp8 actor_kl use=${ACTOR_USE_KL_LOSS} coef=${ACTOR_KL_LOSS_COEF} type=${ACTOR_KL_LOSS_TYPE}"
echo "StreamWeave exp8 grppo step_filter enable=${GRPPO_FILTER_GROUPS_ENABLE} min_std=${GRPPO_FILTER_MIN_STD}"
echo "StreamWeave exp8 scale train_batch=${TRAIN_BATCH_SIZE} gen_batch=${GEN_BATCH_SIZE} val_batch=${VAL_BATCH_SIZE} val_max_samples=${VAL_MAX_SAMPLES} rollout.n=${ROLLOUT_N} max_steps=${MAX_STEPS}"
